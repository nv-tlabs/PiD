...
from pid._ext.imaginaire.datasets.webdataset.config.schema import TarSample
from pid._ext.imaginaire.datasets.webdataset.utils.stream import RetryingStream
...
# Number of attempts to open/download an object-store tar. RetryingStream also
# retries individual HEAD/GET/read calls; keep this outer retry bounded so one
# bad shard cannot pin a prefetch worker for many minutes.
_NUM_OBJECT_STORE_READ_ATTEMPTS = max(1, _env_int("IMAGINAIRE_OBJECT_STORE_READ_ATTEMPTS", 2))
