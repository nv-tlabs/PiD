...
from pid._ext.imaginaire.flags import INTERNAL
from pid._ext.imaginaire.utils.context_managers import distributed_init
...
class ImaginaireTrainer:
    ...
    def train(
        ...
        self.optimizer = optimizer  # expose to callbacks for memory management (e.g. CPU offload during eval)
        ...
        if grad_accum_iter == self.config.trainer.grad_accum_iter:
            with self.training_timer("optimizer_step"):
                with self.straggler_detector.profile_section(
                    ...
                    # Support dynamic optimizer switching for distillation models
                    # If model has get_current_optimizer_scheduler, use it to get the correct optimizer/scheduler
                    # for the current iteration (e.g., student vs fake_score in DMD distillation)
                    if self.config.trainer.distributed_parallelism == "ddp":
                        _model = model_ddp.module
                    else:
                        _model = model_ddp

                    # For distillation models, get the correct optimizer/scheduler for the current iteration
                    if hasattr(_model, "get_current_optimizer_scheduler"):
                        current_optimizer, current_scheduler = _model.get_current_optimizer_scheduler(iteration)
                    else:
                        current_optimizer, current_scheduler = optimizer, scheduler

                    self.callbacks.on_before_optimizer_step(
                        model_ddp, current_optimizer, current_scheduler, grad_scaler, iteration=iteration
                    )
                    grad_scaler.step(current_optimizer)
                    grad_scaler.update()
