...
import pid._ext.imaginaire.utils.easy_io.backends.auto_auth as auto
...
# Keep object-store calls bounded. With many ranks/workers, botocore's old
# 20-attempt adaptive retry could keep a stuck GCS HEAD/GET inside one
# prefetch thread for many minutes, which then stalls all DDP ranks.
GLOBAL_S3_CONFIG = Config(
    retries={
        "max_attempts": _env_int("IMAGINAIRE_S3_MAX_ATTEMPTS", 3),
        "mode": os.environ.get("IMAGINAIRE_S3_RETRY_MODE", "standard"),
    },
    connect_timeout=_env_float("IMAGINAIRE_S3_CONNECT_TIMEOUT", 5.0),
    read_timeout=_env_float("IMAGINAIRE_S3_READ_TIMEOUT", 15.0),
    max_pool_connections=_env_int("IMAGINAIRE_S3_MAX_POOL_CONNECTIONS", 64),
)
Image.MAX_IMAGE_PIXELS = None

if TYPE_CHECKING:
    from pid._ext.imaginaire.config import ObjectStoreConfig
...
class ObjectStore:
    ...
    def load_object(
        ...
        for attempt in range(max_attempts):
            ...
            # ClientError covers HTTP-level failures (4xx/5xx); BotoCoreError covers the
            # transport layer — SSLError / EndpointConnectionError / ConnectionClosedError.
            # GCS intermittently resets TLS connections under the burst of concurrent wdinfo
            # downloads at startup, raising botocore SSLError (a BotoCoreError, NOT a
            # ClientError). Without catching it here that error escaped the retry loop and
            # crashed dataset parsing, taking down the whole job.
