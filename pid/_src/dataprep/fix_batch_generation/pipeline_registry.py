...
Supported backbones: flux, sdxl, sd3, flux2, qwenimage, qwenimage_2512, zimage.
...
PIPELINE_REGISTRY: dict[str, DiffusionPipelineConfig] = {
    ...
    # Dec 2025 refresh of Qwen-Image — same AutoencoderKLQwenImage (per-channel
    # mean/std identical to Wan2.1 VAE) and same QwenImagePipeline class, only
    # the transformer checkpoint differs.
...
def to_training_frame(latent: torch.Tensor, sigma: float, cfg: DiffusionPipelineConfig) -> tuple[torch.Tensor, float]:
    """Map an inference-loop intermediate latent + its scheduler σ to the frame our
    training-time noiser produces.

    Most backbones in the registry (Flux / Flux2 / SD3 / QwenImage / ZImage) use
    flow-matching schedulers: σ ∈ [0,1] and x_t = (1-σ) x_0 + σ ε. This matches
    `LatentNoiser(backbone="flow_matching")` in
    pid/_src/models/pixeldit/latent_noising.py directly — no conversion.

    SDXL is the exception. Its default `EulerDiscreteScheduler` (or any DDPM-derived
    scheduler used at inference) holds latents in the variance-exploding Euler/Karras
    frame:
        x_t^eu = x_0 + σ_eu ε,    σ_eu = sqrt((1 - α̅_t) / α̅_t),    σ_eu ∈ [0, ~15]
    Our training-time `LatentNoiser(backbone="sdxl")` instead produces the VP/DDPM
    frame:
        x_t^vp = sqrt(α̅_t) x_0 + sqrt(1 - α̅_t) ε,                σ_vp ∈ [0, 1]
    The two are related by a single scalar (same noise realization, different scale):
        x_t^vp = x_t^eu / sqrt(σ_eu² + 1),   σ_vp = σ_eu / sqrt(σ_eu² + 1)
    Equivalently `1 / sqrt(σ_eu² + 1) = sqrt(α̅_t)` — what `scale_model_input` returns.

    Returns (latent_in_training_frame, sigma_in_training_frame) with the same dtype/device
    as the input. σ is a Python float so it can be written into caption JSON as-is.
    """
    if cfg.name == "sdxl":
...
def extract_latent(pipeline, raw_output, cfg: DiffusionPipelineConfig, height: int, width: int) -> torch.Tensor:
    ...
    if cfg.name == "flux":
        ...
        # QwenImage / QwenImage-2512: packed (B, seq_len, C) → (B, C, 1, H, W) with temporal dim
