...
class DPM_Solver:
    ...
    def dpm_solver_adaptive(
        ...
        def _lower_update_order2(x, s, t):
            return self.dpm_solver_first_update(x, s, t, return_intermediate=True)

        def _higher_update_order2(x, s, t, **kwargs):
            return self.singlestep_dpm_solver_second_update(x, s, t, r1=0.5, solver_type=solver_type, **kwargs)

        def _lower_update_order3(x, s, t):
            return self.singlestep_dpm_solver_second_update(
                x, s, t, r1=1.0 / 3.0, return_intermediate=True, solver_type=solver_type
            )

        def _higher_update_order3(x, s, t, **kwargs):
            return self.singlestep_dpm_solver_third_update(
                x, s, t, r1=1.0 / 3.0, r2=2.0 / 3.0, solver_type=solver_type, **kwargs
            )

        if order == 2:
            r1 = 0.5
            lower_update = _lower_update_order2
            higher_update = _higher_update_order2
        elif order == 3:
            r1, r2 = 1.0 / 3.0, 2.0 / 3.0
            lower_update = _lower_update_order3
            higher_update = _higher_update_order3
        else:
