...
class OperationWatchdog:
    ...
    def watch(self, operation_name: str, description: str = "", verbose_first_n: int = -1) -> Iterator[None]:
        ...
        try:
            ...
            op_desc = description or operation_name
            if duration > self._warning_threshold:
                log.warning(
                    f"[{self._name}] SLOW OPERATION: name='{operation_name}', desc='{op_desc}', "
                    f"duration={duration:.2f}s, threshold={self._warning_threshold:.2f}s, id={op_id}",
                    rank0_only=False,
                )
            ...
            if verbose_first_n > 0 and stats["count"] <= verbose_first_n:
                ...
                log.info(
                    f"[{self._name}] operation='{operation_name}', desc='{op_desc}', "
                    f"sample={stats['count']}/{verbose_first_n}, duration={duration:.2f}s, "
                    f"avg={avg_time:.2f}s, max={stats['max_time']:.2f}s, last={stats['last_time']:.2f}s",
                    rank0_only=False,
                )
            ...
            with self._lock:
                for _, op in list(self._ops.items()):
                    ...
                    if elapsed > self._warning_threshold and not op["warned"]:
                        ...
                        log.warning(
                            f"[{self._name}] POSSIBLE STALL: active_operation='{op['name']}', "
                            f"desc='{op['desc']}', running={total_time:.1f}s, "
                            f"idle_since_heartbeat={elapsed:.1f}s, threshold={self._warning_threshold:.1f}s",
                            rank0_only=False,
                        )
                        op["warned"] = True
