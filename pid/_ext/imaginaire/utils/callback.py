    ...
    def __init__(self, config: Config, trainer: ImaginaireTrainer) -> None:
        ...
        if callback_configs:
            ...
            for callback_name, current_callback_cfg in callback_configs.items():
                # Allow experiment overrides to disable an inherited callback by
                # setting its entry to None (e.g. dropping the parent's heavy
                # `EveryNEvaluate` eval batches from a smoke / 1-step variant).
                if current_callback_cfg is None:
                    log.info(f"Callback {callback_name} explicitly disabled (None override); skipping")
                    continue
...
class WandBCallback(Callback):
    ...
    def on_before_optimizer_step(
        ...
        if iteration % self.config.trainer.logging_iter == 0 and distributed.is_rank0():
            ...
            # Log per-phase LR for distillation models (student vs fake_score have different LR)
            _model = model_ddp.module if hasattr(model_ddp, "module") else model_ddp
            update_type = getattr(_model, "_current_update_type", None)
            if update_type:
                wandb.log({f"optim/lr_{update_type}": lr}, step=iteration)
            else:
                wandb.log({"optim/lr": lr}, step=iteration)
