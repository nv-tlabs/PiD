# Vendored from DeQA-Score/src/model/modeling_mplug_owl2.py.
# Trimmed to the inference-only path: dropped forward_pair / get_score /
# softkl_loss / rating_loss / binary_rating_loss / forward dispatch on
# input_type. Those rely on torch.distributed and are only used during
# training. The single-image `forward_single` path is preserved unchanged
# (renamed back to `forward`) so logits at the last position match upstream.
import os
import sys
...
from transformers import AutoConfig, AutoModelForCausalLM, LlamaForCausalLM, LlamaModel
...
class MPLUGOwl2MetaModel:
    def __init__(self, config):
        ...
        self.vision_model = MplugOwlVisionModel(MplugOwlVisionConfig(**config.visual_config["visual_model"]))
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, attention_mask, past_key_values, labels, images):
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            if (cur_input_ids == IMAGE_TOKEN_INDEX).sum() == 0:
                ...
                # FIXME: hacky fix for deepspeed zero3
...
class MPLUGOwl2LlamaForCausalLM(LlamaForCausalLM, MPLUGOwl2MetaForCausalLM):
    ...
    def forward(
        ...
        **_unused,  # Soak up training-only kwargs (use_softkl_loss, level_probs, ...).
